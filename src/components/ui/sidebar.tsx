      ...
      <Sheet open={openMobile} onOpenChange={setOpenMobile} {...props}>
        ...
        >
          <SheetHeader className="sr-only">
            <SheetTitle>{t("ui.sidebar.sheetTitle")}</SheetTitle>
            <SheetDescription>{t("ui.sidebar.sheetDescription")}</SheetDescription>
          </SheetHeader>
  ...
  return (
    ...
    >
      ...
      <div
        ...
        data-side={side}
        className={cn(
          "fixed top-0 bottom-(--status-bar-height) z-10 hidden w-(--sidebar-width) transition-[left,right,width] duration-200 ease-linear data-[side=left]:left-0 data-[side=left]:group-data-[collapsible=offcanvas]:left-[calc(var(--sidebar-width)*-1)] data-[side=right]:right-0 data-[side=right]:group-data-[collapsible=offcanvas]:right-[calc(var(--sidebar-width)*-1)] md:flex",
...
function SidebarTrigger({
  ...
  return (
    ...
    >
      <PanelLeftIcon />
      <span className="sr-only">{t("ui.sidebar.toggleAriaLabel")}</span>
    </Button>
  )
}
...
function SidebarRail({ className, ...props }: React.ComponentProps<"button">) {
  ...
  return (
    <button
      ...
      aria-label={t("ui.sidebar.toggleAriaLabel")}
      tabIndex={-1}
      onClick={toggleSidebar}
      title={t("ui.sidebar.toggleTitle")}
      className={cn(
